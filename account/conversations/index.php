...
<head>
	...
	<title>My Conversations &#8211; BEMYSPOUSE</title>
...
(function($)
{
    $(document).ready(function()
    {
        ...
        $container.load("getConversation.php");
...
var dataString = 'mess1='+ message + '&myid1='+ myid + '&user1='+ usser + '&code1='+ code;
					...
					<div id="breadcrumbs-wrapp">
						<div class="row">
							<div class="nine columns">
								<ul class="breadcrumbs hide-for-small"><li><a href="../../index.php" title="Sweet Date" rel="home" class="trail-begin">Home</a>  </li><li><a href="#" title="My Conversations">My Conversations</a>  </li></ul>							</div>
...
<aside class="four columns">
	...
	<div class="widgets-container sidebar_location">
		<div id="bp_core_members_widget-4" class="widgets clearfix widget_bp_core_members_widget buddypress widget"><h5>View All Conversations</h5>
		
			<ul id="members-list" class="item-list" aria-live="polite" aria-relevant="all" aria-atomic="true">
				<?php
					$sql_dis = "SELECT DISTINCT code FROM conversations where sender='$smem_id' or receiver='$smem_id' order by 1 desc";
					$run_dis = mysqli_query($con, $sql_dis);
					while($row = mysqli_fetch_array($run_dis)){
						$dis_code=$row['code'];
						
						$sql_nm = "SELECT * FROM conversations where code='$dis_code'";
						...
						if($con_sen == $smem_id){
							$con_user = $con_rec;
						}elseif($con_rec == $smem_id){
							$con_user = $con_sen;
						}

						$sql_mbs = "SELECT * FROM members where id='$con_user'";
						$run_mbs = mysqli_query($con, $sql_mbs);
						while($row = mysqli_fetch_array($run_mbs)){
							$umem_id=$row['id'];
							$uf_name=$row['f_name'];
							$um_name=$row['m_name'];
							$ul_name=$row['l_name'];
						...
						$sqlnew = "select * from conversations where code='$dis_code' and receiver='$smem_id' and status='1'"; 
						$resnew=mysqli_query($con,$sqlnew);
						$countnew = mysqli_num_rows($resnew);
						
						if($countnew > 0){
							$newMess = "$countnew new";
						}else{
							$newMess = "";
						}
						...
						echo "
							<li class='vcard'>
								<a href='index.php?recipient=$umem_id&conversation=$dis_code'>
									<div class='item-avatar'>
										<img src='../../members/photos/$upic' class='avatar user-45447-avatar avatar-120 photo' style='width: 58px; height: 58px;' alt='Profile picture of $uf_name $um_name $ul_name' />
									</div>

									<div class='item'>
										<div class='item-title fn' style='font-size: 13px;'>$uf_name $um_name $ul_name</div>
										<div class='item-meta'>
											<span class='activity' ><b style='color: darkred;'>$newMess</b></span>
										</div>
									</div>
								</a>
							</li>
...
<?php
	if(isset($_GET['conversation'])){
		$recipi = $_GET['recipient'];
		$conversa = $_GET['conversation'];
		$_SESSION['codeid'] = $conversa;

		$qry = "update conversations set status='2' where code='$conversa' and receiver='$smem_id'";
		$run = mysqli_query($con, $qry);

		$sql_reci = "SELECT * FROM members where id='$recipi'";
						$run_reci = mysqli_query($con, $sql_reci);
						while($row = mysqli_fetch_array($run_reci)){
							$umem_id=$row['id'];
							$uf_name=$row['f_name'];
							$um_name=$row['m_name'];
							$ul_name=$row['l_name'];
...
?>
	...
	<div id="item-nav">
		<div class="item-list-tabs no-ajax" id="object-nav" aria-label="Group primary navigation" role="navigation">
			<ul>
				<li id="members-groups-li"  class="current selected" ><a id="members" href=""><?php echo $uf_name." ".$um_name." ".$ul_name; ?></a></li>				
			</ul>
		</div>
	</div><!-- #item-nav -->
...
<div class=" single-group" aria-live="polite" aria-atomic="true" aria-relevant="all">
...
<li class="groups joined_group activity-item mini date-recorded-1443486963" id="activity-4575">
	<div class="" style="padding-left: 5px;">
		<form id="myForm">
			...
			<input type="hidden" id="myid" value="<?php echo $smem_id; ?>">
			<input type="hidden" id="uid" value="<?php echo $recipi; ?>">
			<input type="hidden" id="code" value="<?php echo $conversa; ?>">
